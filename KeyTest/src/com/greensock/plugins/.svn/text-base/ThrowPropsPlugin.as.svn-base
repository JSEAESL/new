...
/**
 * [AS3/AS2 only] <code>ThrowPropsPlugin</code> allows you to simply define an initial velocity for a property 
 ...
 * 
 * <p>For example, let's say a user clicks and drags a ball and you track its velocity using an 
 ...
 * based on whatever ease you define in your tween. </p>
 * 
 * <p>Oh, and one more challenge: maybe you want the final resting value to always lie within a 
 ...
 * (if the initial velocity is fast enough to require that). The whole point is to make it look smooth.</p>
 * 
 ...
 * Also notice that you must define a <code>velocity</code> value for each property.</p>
 * 
 ...
 * it adds several other optional parameters. Read the <a href="#to()">docs below</a> for details.</p>
 * 
 ...
 * 
 * <p>A unique convenience of ThrowPropsPlugin compared to most other solutions out there which use 
 ...
 * 
 * <p><i>(note: it is best to use an <code>easeOut</code> with <code>throwProps</code> tweens, but 
 * you can vary the strength by using different flavors like <code>Strong.easeOut, Cubic.easeOut, 
 * Quad.easeOut, Back.easeOut</code>, etc.)</i></p>
 * 
 ...
 * rectangle, smoothly easing into place regardless of where and how fast it is thrown:</p>
 * <listing version="3.0">
...
ThrowPropsPlugin.to(mc, {throwProps:{x:{velocity:500, max:800, min:0}, y:{velocity:-300, max:700, min:100}, resistance:150}});
		...
		override public function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			...
			var p:String, curProp:Object, velocity:Number, change1:Number, curVal:Number, isFunc:Boolean, end:Number, change2:Number, duration:Number = tween._duration, cnt:uint = 0;
			for (p in value) {
				if (p != "resistance" && p != "checkpoint") {
					curProp = value[p];
					if (typeof(curProp) == "number") {
		...
		override public function setRatio(v:Number):void {
			var i:int = _props.length, cp:ThrowProp, val:Number;
			while (--i > -1) {
				cp = _props[i];
